...
# nnet players
n1 = NNet(g)
n1.load_checkpoint(current_directory+"\yoxii\models\checkpoints\\",'best.pth.tar')
args1 = dotdict({'numMCTSSims': 50, 'cpuct':1.0})
mcts1 = MCTS(g, n1, args1)
n1p = lambda x: np.argmax(mcts1.getActionProb(x, temp=0)) # exchange with rp if you dont want to lose

n2 = NNet(g)
n2.load_checkpoint(current_directory+"\yoxii\models\checkpoints\\",'best.pth - copy3.6.24.tar')
args2 = dotdict({'numMCTSSims': 50, 'cpuct':1.0})
mcts2 = MCTS(g, n2, args2)
n2p = lambda x: np.argmax(mcts2.getActionProb(x, temp=0)) # exchange with rp if you dont want to lose


player2 = n2p # Exchange with rp (random player), hp (human player)
