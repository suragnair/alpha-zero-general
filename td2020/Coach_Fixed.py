...
Has changed model choosing function from previously defined Coach class
...
class Coach:
    ...
    def __init__(self, game, nnet, args):
        self.game = game
        self.nnet = nnet
        self.pnet = self.nnet.__class__(self.game, CONFIG.nnet_args.encoder)  # the competitor network
        self.args = args
    ...
    def learn(self):
        ...
        for i in range(1, self.args.numIters + 1):
            ...
            pwins, nwins, draws = arena.playGames(self.args.arenaCompare, False, i)

            print('NEW/PREV WINS : %d / %d ; DRAWS : %d' % (nwins, pwins, draws))
            if pwins + nwins == 0 or float(nwins) / (pwins + nwins) < self.args.updateThreshold:
                # print("custom model selection formula nwins <= pwins + draws? reject: accept")
                # if nwins <= pwins + draws:
